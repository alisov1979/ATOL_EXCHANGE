...
public class Map {
	...
	public BigDecimal getDecimal(String value) {
		
		Property property = getPropertyByName(value);

		try {
			return BigDecimal.valueOf(Double.valueOf(property.getValue().replace(",", ".")));
		} catch (NullPointerException | NumberFormatException e) {
	...
	public String getString(String value, int lenght) {
		
		Property property = getPropertyByName(value);

		if (property != null) {

			return property.getValue().substring(0, Math.min(property.getValue().length(), lenght -1));
	...
	public java.sql.Date getSQLDate(String value ) {
		...
		java.sql.Date date = new java.sql.Date(dt.getYear(), dt.getMonth(), dt.getDay());

		return date;

	}
	
	public static java.sql.Date SQLDate(String value ) {
